    ...
    pub fn parse(packet: &Packet) -> Result<Self> {
        ...
        let accept_data = if data_length > 0 && data_offset > 0 {
            ...
            if buf.has_remaining(data_length) {
                ...
                Some(String::from_utf8(data_bytes).map_err(|_| {
                    Error::DataConversionError("Oracle accept data is not valid UTF-8".to_string())
                })?)
            } else {
                None
            }
...
mod tests {
    ...
    #[test]
    fn test_parse_accept_rejects_invalid_utf8_data() {
        let payload = [
            0x01, 0x3B, // Protocol version: 315
            0x00, 0x01, // Service options
            0x20, 0x00, // SDU: 8192
            0xFF, 0xFF, // TDU: 65535
            0x00, 0x00, // Hardware byte order
            0x00, 0x01, // Data length: 1
            0x00, 0x1C, // Data offset: 28
            0x00, 0x00, // Flags
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Reserved
            0x00, 0x00, 0x20, 0x00, // SDU 32-bit: 8192
            0xff, // Invalid UTF-8 accept data
        ];

        let packet = make_accept_packet(&payload);
        assert!(matches!(
            AcceptMessage::parse(&packet),
