...
const MAX_SERVER_BANNER_BYTES: usize = 4096;
...
impl ProtocolMessage {
    ...
    fn read_null_terminated_string(buf: &mut ReadBuffer) -> Result<String> {
        ...
        String::from_utf8(bytes).map_err(|_| {
            Error::DataConversionError("Oracle server banner is not valid UTF-8".to_string())
        })
    }
}
...
mod tests {
    ...
    fn test_build_protocol_request() {
        ...
        assert!(packet
            .windows(DRIVER_NAME.len())
            .any(|window| window == DRIVER_NAME.as_bytes()));
    }
    ...
    #[test]
    fn test_server_banner_is_bounded_and_strict_utf8() {
        let mut invalid = ReadBuffer::from_slice(&[0xff, 0]);
        assert!(matches!(
            ProtocolMessage::read_null_terminated_string(&mut invalid),
            Err(Error::DataConversionError(_))
        ));

        let mut oversized = vec![b'A'; MAX_SERVER_BANNER_BYTES + 1];
        oversized.push(0);
        let mut oversized = ReadBuffer::from_slice(&oversized);
        assert!(matches!(
            ProtocolMessage::read_null_terminated_string(&mut oversized),
