...
impl RedirectMessage {
    ...
    pub fn parse(packet: &Packet) -> Result<Self> {
        ...
        let data = String::from_utf8(data_bytes).map_err(|_| {
            Error::DataConversionError("Oracle redirect data is not valid UTF-8".to_string())
        })?;
...
mod tests {
    ...
    #[test]
    fn test_parse_redirect_rejects_invalid_utf8() {
        let payload = [0x00, 0x01, 0xff];
        let packet = make_redirect_packet(&payload);

        assert!(matches!(
            RedirectMessage::parse(&packet),
