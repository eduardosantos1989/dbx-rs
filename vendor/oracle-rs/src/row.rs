        ...
        match self.read_oracle_slice(buf)? {
            ...
            Some(data) => {
                ...
                if data[0] == 1 && data.len() >= 13 {
                    ...
                    let s = String::from_utf8(data[1..].to_vec()).map_err(|_| {
                        Error::DataConversionError(
                            "Oracle universal ROWID is not valid UTF-8".to_string(),
                        )
                    })?;
                    Ok(Value::String(s))
                }
...
mod tests {
    ...
    #[test]
    fn test_decode_logical_urowid_rejects_invalid_utf8() {
        let decoder = RowDataDecoder::new(&[]);
        let mut buf = ReadBuffer::from_slice(&[2, 2, 0xff]);

        assert!(matches!(
            decoder.decode_urowid(&mut buf),
    ...
    #[test]
    fn duplicate_without_previous_row_is_a_protocol_error() {
        let columns = vec![make_column("COL1", OracleType::Varchar, 10)];
        let mut decoder = RowDataDecoder::new(&columns);
        decoder.set_bit_vector(vec![0]);
        let mut buffer = ReadBuffer::from_slice(&[]);

        assert!(matches!(
            decoder.decode_row(&mut buffer, None),
