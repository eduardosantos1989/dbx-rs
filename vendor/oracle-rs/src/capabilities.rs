...
impl Capabilities {
    ...
    /// Check that negotiated text encodings match the lossless decoders.
    pub fn check_text_encodings(&self) -> crate::error::Result<()> {
        if self.charset_id != charset::AL32UTF8 {
            return Err(crate::error::Error::FeatureNotSupported(format!(
                "database character set {} is not supported (only AL32UTF8)",
                self.charset_id
            )));
        }
        if self.ncharset_id != charset::AL16UTF16 {
            return Err(crate::error::Error::FeatureNotSupported(format!(
                "national character set {} is not supported (only AL16UTF16)",
                self.ncharset_id
            )));
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn test_check_text_encodings_valid() {
        let mut caps = Capabilities::new();

        caps.charset_id = charset::AL32UTF8;
        caps.ncharset_id = charset::AL16UTF16;
        assert!(caps.check_text_encodings().is_ok());
    }

    #[test]
    fn test_check_text_encodings_invalid() {
        let mut caps = Capabilities::new();

        caps.charset_id = 31;
        assert!(caps.check_text_encodings().is_err());

        caps.charset_id = charset::AL32UTF8;
        caps.ncharset_id = charset::AL32UTF8;
        assert!(caps.check_text_encodings().is_err());
    }
