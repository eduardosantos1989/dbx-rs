...
impl ReadBuffer {
    ...
    pub fn read_bytes_with_length(&mut self) -> Result<Option<Vec<u8>>> {
        self.read_bytes_with_length_bounded(usize::MAX)
    }

    /// Read a TNS length-prefixed byte sequence with a cumulative size limit.
    ///
    /// Chunked values are rejected after their length header and before the
    /// payload that would exceed `maximum` is read or copied.
    pub fn read_bytes_with_length_bounded(&mut self, maximum: usize) -> Result<Option<Vec<u8>>> {
        let mut result = Vec::new();
        if self.consume_bytes_with_length_bounded(maximum, Some(&mut result))? {
            Ok(Some(result))
        } else {
            Ok(None)
        }
    ...
    /// Skip a TNS length-prefixed byte sequence with a cumulative size limit.
    ///
    /// This applies the same framing and admission checks as
    /// [`Self::read_bytes_with_length_bounded`] without allocating a value.
    pub fn skip_bytes_with_length_bounded(&mut self, maximum: usize) -> Result<()> {
        self.consume_bytes_with_length_bounded(maximum, None)
            .map(|_| ())
    }

    fn consume_bytes_with_length_bounded(
        &mut self,
        maximum: usize,
        ...
        if len == length::LONG_INDICATOR {
            ...
            loop {
                ...
                self.ensure_remaining(chunk_len)?;
                let end = self.pos + chunk_len;
                if let Some(result) = output.as_deref_mut() {
                    result
                        .try_reserve_exact(chunk_len)
                        .map_err(|_| Error::LimitExceeded)?;
                    result.extend_from_slice(&self.data[self.pos..end]);
                }
                self.pos = end;
            }
            return Ok(true);
        ...
        self.ensure_remaining(actual_len)?;
        let end = self.pos + actual_len;
        if let Some(result) = output.as_deref_mut() {
            result
...
mod tests {
    ...
    #[test]
    fn bounded_length_rejects_short_value_before_reading_payload() {
        let mut buf = ReadBuffer::from_slice(&[0x03, 0x41, 0x42, 0x43]);

        assert!(matches!(
            buf.read_bytes_with_length_bounded(2),
    ...
    #[test]
    fn bounded_length_rejects_chunk_before_copying_overflow() {
        ...
        assert!(matches!(
            buf.read_bytes_with_length_bounded(4),
            Err(Error::LimitExceeded)
        ));
        assert_eq!(buf.position(), 8);
        assert_eq!(buf.remaining_bytes(), &[0x44, 0x45, 0x00]);
    }

    #[test]
    fn bounded_length_accepts_exact_chunked_limit() {
        let mut buf = ReadBuffer::from_slice(&[
            length::LONG_INDICATOR,
            0x01,
            0x03,
        ...
        let bytes = buf.read_bytes_with_length_bounded(5).unwrap().unwrap();
    ...
    #[test]
    fn bounded_skip_rejects_chunk_before_consuming_overflow() {
        let mut buf = ReadBuffer::from_slice(&[
            length::LONG_INDICATOR,
            0x01,
            0x03,
        ...
        assert!(matches!(
            buf.skip_bytes_with_length_bounded(4),
            Err(Error::LimitExceeded)
        ));
        assert_eq!(buf.position(), 8);
        assert_eq!(buf.remaining_bytes(), &[0x44, 0x45, 0x00]);
    ...
    #[test]
    fn bounded_skip_accepts_exact_chunked_limit_without_allocation() {
        let mut buf = ReadBuffer::from_slice(&[
            length::LONG_INDICATOR,
            0x01,
            0x03,
        ...
        buf.skip_bytes_with_length_bounded(5).unwrap();
