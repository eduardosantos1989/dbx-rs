...
use cbc::cipher::{block_padding::NoPadding, BlockModeDecrypt, BlockModeEncrypt};
use md5::Md5;
...
fn encrypt_cbc_256_internal(key: &[u8], plaintext: &[u8], use_pkcs7: bool) -> Result<Vec<u8>> {
    ...
    let cipher = Aes256CbcEnc::new_from_slices(key, &ZERO_IV)
        .map_err(|_| Error::Protocol("invalid AES-256 key or IV length".to_string()))?;
    let ciphertext = cipher
        .encrypt_padded::<NoPadding>(&mut buffer, total_len)
...
pub fn decrypt_cbc_256(key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>> {
    ...
    let cipher = Aes256CbcDec::new_from_slices(key, &ZERO_IV)
        .map_err(|_| Error::Protocol("invalid AES-256 key or IV length".to_string()))?;
...
pub fn encrypt_cbc_192(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    let cipher = Aes192CbcEnc::new_from_slices(key, &ZERO_IV)
        .map_err(|_| Error::Protocol("invalid AES-192 key or IV length".to_string()))?;
    let ciphertext = cipher
        .encrypt_padded::<NoPadding>(&mut buffer, msg_len + padding_len)
        .map_err(|e| Error::Protocol(format!("AES encryption failed: {}", e)))?;
...
pub fn decrypt_cbc_192(key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>> {
    ...
    let cipher = Aes192CbcDec::new_from_slices(key, &ZERO_IV)
        .map_err(|_| Error::Protocol("invalid AES-192 key or IV length".to_string()))?;
    let mut buffer = ciphertext.to_vec();
    let plaintext = cipher
        .decrypt_padded::<NoPadding>(&mut buffer)
