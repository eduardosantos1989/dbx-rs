...
pub enum Error {
    ...
    /// Configured wire bounds are internally inconsistent.
    #[error("invalid TNS wire limits")]
    InvalidLimits,

    /// A bounded packet, response, value, row, or column limit was exceeded.
    #[error("TNS response exceeded a configured limit")]
    LimitExceeded,

    /// More TNS packets are required to complete this response.
    #[error("incomplete TNS response")]
    IncompleteResponse,
