...
impl OsonDecoder {
    ...
    fn read_short_field_names(
        ...
        for offset in offsets {
            ...
            let name = String::from_utf8(name_bytes.to_vec()).map_err(|_| {
                Error::DataConversionError("Oracle OSON field name is not valid UTF-8".to_string())
            })?;
            names.push(name);
        }
    ...
    fn read_string(&mut self, len: usize) -> Result<String> {
        ...
        String::from_utf8(bytes).map_err(|_| {
            Error::DataConversionError("Oracle OSON string is not valid UTF-8".to_string())
        })
    }
}
...
mod tests {
    ...
    #[test]
    fn test_oson_string_rejects_invalid_utf8() {
        let mut decoder = OsonDecoder {
