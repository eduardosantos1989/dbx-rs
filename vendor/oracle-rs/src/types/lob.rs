...
impl LobLocator {
    ...
    pub fn get_file_name(&self) -> Option<(String, String)> {
        ...
        let dir_name = String::from_utf8(
            self.locator[dir_name_offset..dir_name_offset + dir_name_len].to_vec(),
        )
        .ok()?;
        ...
        let file_name = String::from_utf8(
            self.locator[file_name_offset..file_name_offset + file_name_len].to_vec(),
        )
        .ok()?;
...
impl LobValue {
    ...
    pub fn as_string(&self) -> Result<Option<String>> {
        match self {
            ...
            Self::Inline(data) => String::from_utf8(data.to_vec()).map(Some).map_err(|_| {
...
mod tests {
    ...
    #[test]
    fn test_lob_text_rejects_invalid_utf8() {
        let lob = LobValue::inline(Bytes::from_static(&[0xff]));
        assert!(matches!(
            lob.as_string(),
            Err(Error::DataConversionError(_))
        ));

        let mut locator_bytes = vec![0u8; 16];
        locator_bytes.extend_from_slice(&[0, 1, 0xff, 0, 0]);
