...
/// Maximum canonical Oracle NUMBER wire length: one exponent and twenty base-100 digits.
const MAX_ENCODED_BYTES: usize = 21;
...
pub fn decode_oracle_number(data: &[u8]) -> Result<OracleNumber> {
    ...
    if data.len() > MAX_ENCODED_BYTES {
        return Err(Error::DataConversionError(
            "Oracle NUMBER exceeds its canonical wire length".to_string(),
        ));
    }
    ...
    if data.len() == 1 {
        ...
        return Err(Error::DataConversionError(
            "Oracle NUMBER contains an invalid single-byte encoding".to_string(),
        ));
    }
    ...
    if mantissa_len == 0
        || data[1..=mantissa_len].iter().any(|byte| {
            if is_positive {
                !(1..=100).contains(byte)
            } else {
                !(2..=101).contains(byte)
    ...
    {
        return Err(Error::DataConversionError(
            "Oracle NUMBER contains an invalid base-100 digit".to_string(),
        ));
    }
...
mod tests {
    ...
    #[test]
    fn decode_rejects_noncanonical_length_and_digits() {
        assert!(decode_oracle_number(&[0xc1; MAX_ENCODED_BYTES + 1]).is_err());
        assert!(decode_oracle_number(&[0xc1, 0]).is_err());
        assert!(decode_oracle_number(&[0x3e, 102, 2]).is_err());
        assert!(decode_oracle_number(&[0x81]).is_err());
    }
