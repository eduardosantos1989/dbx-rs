...
pub fn decode_oracle_date(data: &[u8]) -> Result<OracleDate> {
    if data.len() != 7 {
        return Err(Error::DataConversionError(format!(
            "Oracle DATE requires exactly 7 bytes, got {}",
            data.len()
        )));
    }
    ...
    if year == 0
        || !(-4712..=9999).contains(&year)
        || !(1..=12).contains(&data[2])
        || !(1..=31).contains(&data[3])
        || !(1..=24).contains(&data[4])
        || !(1..=60).contains(&data[5])
        || !(1..=60).contains(&data[6])
    {
        return Err(Error::DataConversionError(
            "Oracle DATE contains an invalid encoded component".to_string(),
        ));
    }
...
pub fn decode_oracle_timestamp(data: &[u8]) -> Result<OracleTimestamp> {
    if !matches!(data.len(), 7 | 11 | 13) {
        return Err(Error::DataConversionError(format!(
            "Oracle TIMESTAMP requires 7, 11, or 13 bytes, got {}",
            data.len()
        )));
    }

    let date = decode_oracle_date(&data[..7])?;

    // Fractional seconds (bytes 7-10)
    let fsecond = if data.len() >= 11 {
        let nanos = u32::from_be_bytes([data[7], data[8], data[9], data[10]]);
        if nanos > 999_999_999 || nanos % 1000 != 0 {
            return Err(Error::DataConversionError(
                "Oracle TIMESTAMP cannot be represented exactly at microsecond precision"
                    .to_string(),
            ));
        }
        nanos / 1000
    ...
    let (tz_hour, tz_minute) = if data.len() == 13 {
        if data[11] == 0 || data[12] == 0 {
            return Err(Error::DataConversionError(
                "Oracle TIMESTAMP timezone contains an invalid encoded component".to_string(),
            ));
        }
        ...
        let hour = i16::from(data[11]) - i16::from(TZ_HOUR_OFFSET);
        let minute = i16::from(data[12]) - i16::from(TZ_MINUTE_OFFSET);
        if !(-12..=14).contains(&hour) || !(-59..=59).contains(&minute) {
            return Err(Error::DataConversionError(
                "Oracle TIMESTAMP timezone is outside the supported offset range".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn timestamp_rejects_sub_microsecond_and_malformed_lengths() {
        let mut sub_microsecond = vec![120, 124, 3, 15, 15, 31, 46];
        sub_microsecond.extend_from_slice(&123_456_001_u32.to_be_bytes());
        assert!(decode_oracle_timestamp(&sub_microsecond).is_err());

        assert!(decode_oracle_timestamp(&[120, 124, 3, 15, 15, 31, 46, 0]).is_err());
    }

    #[test]
    fn date_and_timezone_reject_malformed_encoded_components() {
        assert!(decode_oracle_date(&[120, 124, 3, 15, 0, 31, 46]).is_err());

        let mut timestamp = vec![120, 124, 3, 15, 15, 31, 46];
        timestamp.extend_from_slice(&[0, 0, 0, 0, 0, 90]);
        assert!(decode_oracle_timestamp(&timestamp).is_err());
    }
